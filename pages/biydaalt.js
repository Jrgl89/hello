  ...
  return (
    <div className="h-screen w-screen bg-white p-8 flex flex-col items-center overflow-y-scroll overflow-x-hidden">
      <div className="w-full h-1/6 flex justify-center items-center">
        <h1 className="text-4xl mx-5 w-1/5 text-black font-bold">
          Students list
        </h1>
        <div className="flex  items-start justify-center h-1/5 my-6  w-screen">
          <input
            ...
            className="bg-gray-300 transition ease-out delay-250 hover:shadow-md hover:shadow-gray-500 rounded-2xl px-3 py-2 w-1/2"
          ></input>
        </div>
        <button
          className="bg-black flex justify-center items-center transition ease-out delay-250 hover:shadow-md hover:shadow-gray-500 text-white w-32 h-12  border rounded-xl"
          onClick={() => {
            setGrid(!grid);
          }}
        >
      ...
      <div className=" mx-5 w-full h-5/6 flex flex-col items-center">
        <div
          className={
            grid == true
              ? " items-center  grid-cols-3 grid gap-6 w-full "
              : " w-full space-y-4"
          }
        >
          {filteredData.length > 0 ? (
            filteredData.map((filteredData, index) => {
              return (
                <div
                  key={index}
                  className="  bg-gray-100 w-full px-4 py-8 transition ease-out delay-250 hover:shadow-md hover:shadow-gray-500 flex flex-col  rounded-xl text-black h-1/7"
                >
